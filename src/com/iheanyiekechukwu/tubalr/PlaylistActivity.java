...
public class PlaylistActivity extends SherlockActivity implements
		OnClickListener, MyResultReceiver.Receiver, OnItemClickListener,
		OnSeekBarChangeListener, OnCompletionListener,
		OnBufferingUpdateListener, OnPreparedListener, Callback,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * if (savedInstanceState != null) { videoList = (ArrayList<VideoClass>)
		...
		listDivider = findViewById(R.id.listDivider);
		...
		Typeface bolded = Typeface.createFromAsset(context.getAssets(),
		 ...
		 * "android");
		 * 
		 * TextView actionBarTextView = (TextView)
		 * findViewById(actionBarTitleId);
		 * actionBarTextView.setTextColor(Color.WHITE);
		 */

		Typeface bookman = Typeface.createFromAsset(getAssets(),
				"fonts/bookos.ttf");
		// actionBarTextView.setTypeface(bookman);
		getSupportActionBar().setTitle(R.string.tubalr);
		...
		// Adapter has been loaded properly . . . Sooo, yeah, play the first
		// song.

		/*
		 * vidV = (SurfaceView) findViewById(R.id.videoStream); sh =
		 * vidV.getHolder(); sh.setFixedSize(width, newHeight);
		 * sh.addCallback(this);
		 */
		...
		if (newInstance) {
			...
			playlistStringAdapter = new ArrayAdapter<VideoClass>(context,
			...
			pd = ProgressDialog.show(this, "Building Playlist",
			...
			if (musicServiceIntent == null) {
				...
				bindService(musicServiceIntent, serviceConnection,
						Context.BIND_AUTO_CREATE);
			}

			/*
			 * musicServiceBroadcastReceiver = new
			 * MusicServiceBroadcastReceiver(); IntentFilter filter = new
			 * IntentFilter(MusicService.UPDATE_PLAYLIST);
		...
		/*
		 * while(it.hasNext()) { VideoClass currentVideo = it.next();
		 * 
		 * String info = currentVideo.getTitle(); stringList.add(info);
		 * playlistStringAdapter.add(info);
		 * playlistStringAdapter.notifyDataSetChanged(); Log.d("ADD", "Added " +
		 * info + " to list."); }
		 */
	...
	Runnable mStatusChecker = new Runnable() {
		...
		public void run() {
			...
			while (MusicService.isPlaying()) {
				...
				// serviceHandler.postDelayed(mStatusChecker,
				// musicService.maxDuration());
			}
	...
	public void performLogout() {
		String deleteURL = "http://www.tubalr.com/api/sessions/" + UserHelper.userInfo[UserHelper.TOKEN];
		...
		client.setBasicAuth("auth_token", UserHelper.userInfo[UserHelper.TOKEN]);
		client.delete(deleteURL, new JsonHttpResponseHandler() {
			@Override
			public void onSuccess(JSONObject result) {
				if(result.has("token")) {
					Toast.makeText(getBaseContext(), "Successfully logged out.", Toast.LENGTH_SHORT).show();
					UserHelper.userInfo[UserHelper.TOKEN] = "";
					UserHelper.userInfo[UserHelper.USER] = "-1";
				}
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		logout.setVisible(UserHelper.userLoggedIn());
		login.setVisible(!UserHelper.userLoggedIn());
	...
	private void showAddDialog() {
		...
		if (userPlaylists.size() != 0) {
			...
			String[] names = userPlaylists.toArray(new String[userPlaylists
					.size()]);
			...
			.setAdapter(simpleAdapter, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					String currentVideoID = musicService.getCurrentVideo()
							.getId();
					String currentVideoTitle = musicService.getCurrentVideo()
							.getTitle();
					AsyncHttpClient client = new AsyncHttpClient();
					RequestParams params = new RequestParams();
					...
					stringBuilder
							.append("http://www.tubalr.com/api/playlist/add_video?playlist_id=");
					...
					String url = URLEncoder
							.encode("http://www.tubalr.com/api/playlist/add_video?playlist_id="
									+ playlistIDs.get(which)
									+ "video_id="
									+ musicService.getCurrentVideo().getId()
									+ "&video_title="
									+ musicService.getCurrentVideo().getTitle());
					client.post(addURL, params, new JsonHttpResponseHandler() {
						...
						public void onSuccess(JSONObject result) {
							...
							if (added) {
								...
								Toast.makeText(getBaseContext(),
										"Error adding song to the playlist.",
										Toast.LENGTH_SHORT).show();
							}
						}
					});
		...
		case 2:
			if (resultCode == RESULT_OK) {
				...
				Toast.makeText(context, "Successfully logged in. . .",
						Toast.LENGTH_SHORT).show();
				fetchPlaylists();
				// UserPlaylist.newInstance(UserHelper.userInfo[UserHelper.USER],
				// UserHelper.userInfo[UserHelper.TOKEN]);
			}
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		videoList = (ArrayList<VideoClass>) savedInstanceState
				.getSerializable("videos");
	}
	...
	private void linksComposer(String block, int i, String title, String id) {
		...
		if (urlMatcher.find()) {
			...
			if (sigMatcher.find()) {
				...
				linkToAdd
						.replaceAll("&itag=[0-9][0-9]&signature", "&signature");
				...
				try {
					...
					/*
					 * if(sh != null) { player.setDisplay(sh); }
					 */
	...
	private class PlaylistAdapter extends ArrayAdapter<VideoClass> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = inflater.inflate(R.layout.basicitem, parent,
						false);
				item = new PlaylistItem();
				item.songTextView = (TextView) convertView
						.findViewById(R.id.text1);
				item.rowLayout = (LinearLayout) convertView
			 ...
			 * item.rowLayout.setBackgroundColor(Integer.parseInt("409DBD", 16)
			 * + 0xFF000000);
			 * 
			 * } else {
			 * item.rowLayout.setBackgroundColor(Integer.parseInt("409DBD", 16)
	...
	public void onPrepared(MediaPlayer mp) {
		 ...
		 * 
		 * 
		 * player.start(); player.setOnCompletionListener(this);
		 * 
		 * 
		 * if(player.isPlaying()) { playButton.setVisibility(View.GONE);
	...
	private void updatePlayPanel(final VideoClass video) {
		...
		for (int i = visiblePosition; i < playlistView.getLastVisiblePosition(); ++i) {
			...
			if (vid.getId().trim().equals(video.getId().trim())) {
				...
				View newView = playlistView.getAdapter().getView(i, v,
						playlistView);

				if (newView instanceof Checkable) {
					playlistView.setItemChecked(i, true);
				} else if (getBaseContext().getApplicationInfo().targetSdkVersion >= android.os.Build.VERSION_CODES.HONEYCOMB) {
				...
				// newView.setBackgroundColor(Integer.parseInt("409DBD", 16) +
				// 0xFF000000);
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				/*
				 * Log.v(TAG, "PlaylistActivity: " +
				 * Integer.toString(elapsedMillis) +
				 * Integer.toString(musicService.maxDuration()));
				 * currentTextView.setText(video.getTitle());
				 * 
				 * Log.v(TAG, "The image URL is: " + video.getImageURL());
				 * UrlImageViewHelper.setUrlDrawable(videoImageView,
				 * video.getImageURL(), R.drawable.tubalr_icon);
				 * 
	...
	private class UpdateCurrentTrackTask extends
			AsyncTask<Void, VideoClass, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if (!paused) {
				...
				// Log.v(TAG,
				// "PlaylistActivity: Updating current Track Task junts");
				VideoClass currentVideo = musicService.getCurrentVideo();
	...
	public void onReceiveResult(int resultCode, Bundle resultData) {
		...
		case FINISHED:
			...
			videoList = (ArrayList<VideoClass>) resultData
					.getSerializable("videos");
			artist = resultData.getString("artist");

			playlistView = (ListView) findViewById(R.id.playlistView);
			playlistView.setSelector(R.drawable.highlight_selector);
			adapter = new PlaylistAdapter(context, R.layout.basicitem,
					videoList);
			playlistView.setAdapter(adapter);
			...
			Toast.makeText(getBaseContext(),
					"New Playlist Generated! Attempting to play first song. ",
					Toast.LENGTH_SHORT).show();
			...
			if (musicServiceIntent == null) {
				...
				/*
				 * unbindService(serviceConnection); Log.v(TAG,
				 * "Recreating intent?");
				 * MusicService.setMainActivity(PlaylistActivity.this);
				 * musicServiceIntent = new Intent(this, MusicService.class);
				 * musicServiceIntent.putExtra("videos", videoList);
	...
	public void onPause() {
		...
		/*
		 * if(mReceiver != null) { mReceiver.setReceiver(null); }
		 */
	...
	public void onClick(View v) {
		...
		case R.id.playButton:
			if (musicService != null && !MusicService.isPlaying()) {
				...
				/*
				 * player.start(); pauseButton.setVisibility(View.VISIBLE);
				 * playButton.setVisibility(View.GONE);
				 */
	...
	private void showSearchDialog(String message) {
		...
		alert.setPositiveButton("Just", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (s_artist.length() > 0) {
					s_type = "just";
					try {
						s_url = ECHONEST_SONG_URL
								+ URLEncoder.encode(s_artist, "UTF-8")
								+ ECHONEST_RESULT_URL + "40";
					...
					Toast.makeText(getBaseContext(),
							"Building new playlist . . . ", Toast.LENGTH_LONG)
							.show();
					...
					Toast.makeText(getApplicationContext(),
							"Invalid input into the search box!",
							Toast.LENGTH_SHORT).show();
					// dialog.dismiss();
				}
		...
		alert.setNegativeButton("Similar",
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						BugSenseHandler
								.sendEvent("User searched for similar to "
										+ s_artist + " from Playlist");

						if (s_artist.length() > 0) {
							s_type = "similar";
							try {
								s_url = ECHONEST_SIMILAR_URL
										+ URLEncoder.encode(s_artist, "UTF-8")
										+ ECHONEST_RESULT_URL + "40";
		...
		/*
		 * if(!showAlert.isShowing()) { if(player != null) { player.stop();
		 * player.reset(); }
		 * 
	...
	public void updatePlayQueue() {
		...
		if (updateCurrentTrackTask == null) {
			Log.v(TAG,
					"PlaylistActivity: currently in updateCurrentTrackTask pre-execute");
	...
	private class MusicServiceBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			Log.d("PlaylistActivity",
					"MusicServiceBroadcastReceive.onReceive action =  "
							+ intent.getAction());
	...
	private final class MusicServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName className, IBinder baBinder) {
			...
			musicService = ((MusicService.MusicServiceBinder) baBinder)
					.getService();
			MusicService.setMainActivity(PlaylistActivity.this);
			musicServiceIntent.putExtra("videos", videoList);
	 ...
	 * 
	 * if(fromUser) { Log.d(TAG,
	 * "TimelineChangeListener progress received from user: " + progress);
	 * 
	 ...
	 * }
	 * 
	 * private void scheduleSeek(final int progress) { if (delayedSeekTimer !=
	 * null) { delayedSeekTimer.cancel(); }
	 * 
	 * delayedSeekTimer = new Timer(); delayedSeekTimer.schedule(new TimerTask()
	 * {
	 * 
	 * @Override public void run() { // TODO Auto-generated method stub
	 * Log.d(TAG, "Delayed Seek Timer run!"); musicService.seek(progress);
	 * //updatePlayPanel(musicService.getCurrentVideo()); }
	 * 
	 * 
	 * }, 170); }
	 ...
	 * Auto-generated method stub // updateCurrentTrackTask.pause(); }
	 * 
	 * @Override public void onStopTrackingTouch(SeekBar seekBar) { // TODO
	 * Auto-generated method stub Log.d(TAG,
	 * "Started tracking touch in Timeline Change!");
	...
	private boolean isMyServiceRunning() {
		...
		for (RunningServiceInfo service : manager
			...
			if (MusicService.class.getName().equals(
	...
	public void showToast() {
		Toast.makeText(
				this,
				"Working on this feature still, try searching for something! Sorry!",
				Toast.LENGTH_SHORT).show();
	}
