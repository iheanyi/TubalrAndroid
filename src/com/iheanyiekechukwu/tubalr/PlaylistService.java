...
public class PlaylistService extends IntentService {
		...
		try {
			...
			if (playlist.getSongs().size() > 10) {
				for (Song song : playlist.getSongs()) {
					String search = song.getArtistName() + " - "
							+ song.getTitle();
					String type = "genre";
	...
	public void processSimilarJSON(String result) {
		...
		try {
			...
			if (artists.size() > 0) {
				...
				// String yt_short_url = YOUTUBE_SEARCH_URL +
				// URLEncoder.encode(short_search, "UTF-8") +
				// YOUTUBE_END_FORTY_URL;
		...
		/*
		 * Log.d(TAG, "Attempting to stop service. . . "); stopSelf();
		 * Log.d(TAG, "Trying again. . . "); this.stopSelf(); Log.d(TAG,
		 * "Trying stop foreground. . . "); this.stopForeground(true);
		 */
		// try {

		/*
		 * JSONObject object; object = new JSONObject(result); JSONObject
		 * response = object.getJSONObject("response"); JSONObject status =
		 * response.getJSONObject("status"); Integer code =
		 * status.getInt("code"); Log.d("TEST", Integer.toString(code)); String
		 * message = status.getString("message");
		 * 
		 * 
		 * 
		 * // If the artist doesn't exist or the length of the array is less
		 * than 10, execute YouTube command to fetch songs if(code == 5) {
		 * String short_search = artist; String yt_short_url =
		 * YOUTUBE_SEARCH_URL + URLEncoder.encode(short_search, "UTF-8") +
		 * YOUTUBE_END_FORTY_URL; String html = getPage(yt_short_url); String
		 * type = "youtube"; processYoutubeJSON(html, type); //YoutubeTask
		 * myTask = new YoutubeTask(); //myTask.execute(yt_short_url); }
		 * 
		 * 
		 * 
		 * else { JSONArray artists = response.getJSONArray("artists");
		 * 
		 * for(int i = 0; i < artists.length(); ++i) { JSONObject artist =
		 * artists.getJSONObject(i); String name = artist.getString("name");
		 * 
		 * String yt_url; try { // Fetching Youtube Songs yt_url =
		 * YOUTUBE_SEARCH_URL + URLEncoder.encode(name, "UTF-8") +
		 * YOUTUBE_END_URL; //YoutubeTask myTask = new YoutubeTask();
		 * //myTask.execute(yt_url); String html = getPage(yt_url); String type
		 * = "similar"; processYoutubeJSON(html, type);
		 * 
		 ...
		 * TODO Auto-generated catch block e.printStackTrace(); } Log.d("SIM",
		 * "Similar task finished executing . . .");
		 */
	...
	public void processYoutubeJSON(String result, String type) {
		...
		try {
			...
			for (VideoEntry entry : feed.getEntries()) {
				...
				String thumbnailString = "http://i.ytimg.com/vi/" + id
						+ "/0.jpg";
				 ...
				 * entry
				 * .getMediaGroup().getDescription().toString().contains("live")
				 * ||
				 * entry.getMediaGroup().getDescription().toString().contains(
				 * "concert") || title.trim().equals("") || isUnique(entry)) {
				 * continue; } else { videos.add(newVideo); }
				 */

				if (type.equals("just")) {
					if (isUnique(entry) && isMusic(entry)
							&& isNotCoverOrRemix(entry) && isNotLive(entry)
							&& hasTitle(entry)) {
						// Toast.makeText(this, "Adding new video . . . " +
						// videoTitle + " - " + id, Toast.LENGTH_SHORT).show();
						videos.add(newVideo);
					...
					if (isMusic(entry) && isNotCoverOrRemix(entry)
							&& isNotLive(entry)) {
						// Toast.makeText(this, "Adding new video . . . " +
						// videoTitle + " - " + id, Toast.LENGTH_SHORT).show();
						videos.add(newVideo);
		...
		/*
		 * else if(type.equals("youtube")) { if(isNotUserBanned(entry) &&
		 * isNotBlocked(entry)) { videos.add(newVideo);
		 * 
		 * Log.d("TUB", Integer.toString(videos.size())); Log.d("TUB", id +
		 * " - " + videoTitle); Log.d("TUB", "From the Youtube Type");
		 * 
		 * //break; } }
		 * 
		 * 
		 ...
		 * e.printStackTrace(); }
		 * 
		 * /* try { JSONObject object = new JSONObject(result); JSONObject feed
		 * = object.getJSONObject("feed"); JSONArray entries =
		 * feed.getJSONArray("entry");
		 * 
		 * Log.d("ENTRY", "Length is " + Integer.toString(entries.length()));
		 * for(int i = 0; i < entries.length(); ++i) { JSONObject entry =
		 * entries.getJSONObject(i); JSONObject idNode =
		 * entry.getJSONObject("id"); JSONObject media =
		 * entry.getJSONObject("media$group"); JSONArray mediaThumbnail =
		 * media.getJSONArray("media$thumbnail"); JSONObject thumbnail =
		 * mediaThumbnail.getJSONObject(2); String thumbnailString =
		 * thumbnail.getString("url");
		 * 
		 * Log.v("PlayListService", "Thumbnail URL: " + thumbnailString);
		 * 
		 * String full_string = idNode.getString("$t"); String[] splitArray =
		 * full_string.split(":");
		 * 
		 * String id = splitArray[3]; JSONObject titleNode =
		 * entry.getJSONObject("title"); String videoTitle =
		 * titleNode.getString("$t");
		 * 
		 * 
		 ...
		 * 
		 * 
		 * //if(isUnique(entry) && isNotBlocked(entry) && isMusic(entry) &&
		 * isNotCoverOrRemix(entry) && isNotLive(entry) && hasTitle(entry)) {
		 * 
		 * if(type.equals("just")) { if(isUnique(entry) && isNotBlocked(entry)
		 * && isMusic(entry) && isNotCoverOrRemix(entry) && isNotLive(entry) &&
		 * hasTitle(entry)) { //Toast.makeText(this, "Adding new video . . . " +
		 * videoTitle + " - " + id, Toast.LENGTH_SHORT).show();
		 * videos.add(newVideo);
		 * 
		 * Log.d("TUB", Integer.toString(videos.size())); Log.d("TUB", id +
		 * " - " + videoTitle); Log.d("TUB", "From the Just type");
		 * 
		 * break;
		 * 
		 * } }
		 * 
		 * else if(type.equals("similar")) { if(isNotBlocked(entry) &&
		 * isMusic(entry) && isNotCoverOrRemix(entry) && isNotLive(entry)) {
		 * //Toast.makeText(this, "Adding new video . . . " + videoTitle + " - "
		 * + id, Toast.LENGTH_SHORT).show(); videos.add(newVideo);
		 * 
		 * Log.d("TUB", Integer.toString(videos.size())); Log.d("TUB", id +
		 * " - " + videoTitle); Log.d("TUB", "From the similar type . . . ");
		 * 
		 * break;
		 * 
		 * 
		...
		// If the size of the videos list is 40 or the size of the number of
		// results fetched from the
		...
		try {
			...
			for (int i = 0; i < videos.size(); ++i) {
				if (videos.get(i).getId().equalsIgnoreCase(id)) {
					...
					String arrayTitle = videos.get(i).getTitle().toLowerCase()
							.replaceAll(" *\\([^)]*\\)* ", "")
							.replaceAll("[^a-zA-z ]", "");
					String videoTitle = title.toLowerCase()
							.replaceAll(" *\\([^)]*\\)* ", "")
							.replaceAll("[^a-zA-z ]/", "");
					if (videoTitle.equals(arrayTitle)) {
						// Toast.makeText(this.context, "NOT UNIQUE",
	...
	public boolean checkVideo(JSONObject video) {
		if (isUnique(video) && isNotBlocked(video) && isMusic(video)
				&& isNotCoverOrRemix(video) && isNotLive(video)
				&& hasTitle(video)) {
	...
	public boolean isMusic(JSONObject video) {
		...
		try {
			...
			// Toast.makeText(this.context, type.equals("Music") + " - isMusic",
			// Toast.LENGTH_SHORT).show();
	...
	public boolean isNotCoverOrRemix(VideoEntry entry) {
		...
		if (title.contains("cover") || title.contains("remix")
				|| title.contains("alternate") || title.contains("interview")) {
	...
	public boolean isNotLive(VideoEntry entry) {
		String description = entry.getMediaGroup().getDescription()
				.getPlainTextContent();
		String title = entry.getTitle().getPlainText();

		if (description.contains("live") || description.contains("concert")
	...
	public boolean isNotLive(JSONObject video) {
		try {
			String description = video.getJSONObject("media$group")
	...
	/*
	 * TODO Rather than fetching each song URL/ID immediately, just build up the
	 * array first, then execute it? Could lead to a possible performance
	...
	public void processEchoNestJSON(String result) {
		...
		try {
			 ...
			 * params.setResults(40); Playlist playlist =
			 * echoNest.createBasicPlaylist(params);
			 * 
			 * if(playlist.getSongs().size() > 10) { for(Song song:
			 * playlist.getSongs()) { String search = result + " - " +
			 * song.getTitle(); String type = "just"; Log.d(TAG,
			 * "Processing YouTube JSON for " + search);
			 * processYoutubeJSON(search, type); } }
			 */
			...
			if (artists.size() > 10) {
				...
				Log.d(TAG,
						"Size of Songs: " + Integer.toString(justSongs.size()));
				...
				for (int i = 0; i < justSongs.size(); ++i) {
					...
					// int choice = rand.nextInt(images.size());
					/*
					 * Image image = images.get(choice); String url =
					 * image.getURL(); imgURL = url;
		...
		/*
		 * try { JSONObject object = new JSONObject(result); JSONObject resp =
		 * object.getJSONObject("response"); JSONObject status =
		 * resp.getJSONObject("status"); Integer code = status.getInt("code");
		 * Log.d("TEST", Integer.toString(code)); String message =
		 ...
		 * 
		 * 
		 * else { JSONArray songs = resp.getJSONArray("songs"); Log.d("TEST",
		 * Integer.toString(songs.length()));
		 * 
		 * if(songs.length() <= 10) { String short_search = artist; String
		 * yt_short_url = YOUTUBE_SEARCH_URL + URLEncoder.encode(short_search,
		 * "UTF-8") + YOUTUBE_END_FORTY_URL; String html =
		 * getPage(yt_short_url); String type = "youtube";
		 * processYoutubeJSON(html, type); }
		 * 
		 * else { // For each song, hit Youtube's Public API for(int i = 0; i <
		 * songs.length(); ++i) { if(artistSongList.size() == 40 ||
		 * videos.size() == 40) { Log.d("STOP", "Size reached 40!"); break; }
		 * 
		 * else { JSONObject song = songs.getJSONObject(i); String title =
		 * song.getString("title");
		 * 
		 * String search = artist + " - " + title; artistSongList.add(search);
		 * 
		 * try { String yt_url = YOUTUBE_SEARCH_URL + URLEncoder.encode(search,
		 * "UTF-8") + YOUTUBE_END_URL;
		 * 
		 * //YoutubeTask myTask = new YoutubeTask(); //myTask.execute(yt_url);
		 * 
		 * String html = getPage(yt_url); String type = "just";
		 * processYoutubeJSON(html, type); } catch (UnsupportedEncodingException
	...
	private class YoutubeTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPostExecute(String result) {
			// Toast.makeText(context, "Youtube Post Execute Task",
			// Toast.LENGTH_SHORT).show();
			// prog.dismiss();
			String type = "youtube";
	...
	private class EchoSongTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPostExecute(String result) {
			...
			// prog.dismiss();
			// prog = null;
			// Toast.makeText(context, "EchoNest Post Execute Task",
			// Toast.LENGTH_SHORT);
